...
export default function GraphCanvas() {
  ...
  useEffect(() => {
    api
      ...
      .then((g) => {
        if (!g) {
          setError('graph.json 없음. Tools → Rebuild graph 실행 필요.')
          return
        }
      ...
      style: [
        {
          ...
          style: {
            'background-color': (ele: NodeSingular) =>
              (ele.data('favorited') ? '#d97706' : '#3b82f6'),
            label: 'data(label)',
            'font-size': 9,
            color: '#888',
            'text-valign': 'bottom',
            ...
            width: (ele: NodeSingular) =>
              Math.max(6, Math.min(28, 6 + Math.sqrt(ele.data('points') || 0) * 3)),
        ...
        },
        {
          ...
          style: {
            'line-color': (ele: EdgeSingular) =>
              EDGE_COLORS[ele.data('kind') as EdgeKind] ?? '#ccc',
            width: (ele: EdgeSingular) =>
              Math.max(0.3, Math.min(3, (ele.data('weight') || 1) * 0.6)),
            opacity: 0.5,
  ...
  if (error) return <p>그래프 로드 실패: {error}</p>
